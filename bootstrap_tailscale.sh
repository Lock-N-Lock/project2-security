...
# 목적     : proj-mgmt(VMware) Tailscale 연결 + (Opt2) tailscale0 위 VXLAN 오버레이
...
#
# [연결 구조 한눈에]
#   proj-mgmt(100.x) ── Tailscale 암호화 터널 ── Bastion(100.x)   ← 언더레이(L3, WireGuard)
#        └ 그 위에 vxlan0(10.10.10.1) ─ VNI 100 ─ vxlan0(10.10.10.2)  ← 오버레이(L2, dev=tailscale0)
#   즉 VXLAN 패킷(UDP 4789)이 Tailscale 터널 "안으로" 흘러 암호화됨 = 이중 잠금
# =============================================================

set -euo pipefail

# ──────────────────────────────────────────────────────────────────────
#  환경값 (★ 결정사항(5) 메모: 수업 mgmt 가 172.16.8.0/24 를 이미 광고 중이라
...
# ★수정① : Tailscale 에 등록할 이 머신의 hostname
#   기본값 = 현재 머신의 실제 hostname (팀원마다 자동으로 본인 것 사용 → 하드코딩 제거).
#   고정하고 싶으면 TS_HOSTNAME="proj-mgmt" 처럼 직접 지정도 가능.
TS_HOSTNAME="$(hostname -s)"

# ── VXLAN (Opt 2) 설정 ───────────────────────────────────────
# Bastion 의 Tailscale IP 는 EC2 가 tailnet 에 가입한 뒤에 정해지므로,
# 최초 실행 때는 ENABLE_VXLAN=false 로 Tailscale 만 연결하고,
# Bastion TS IP 확보 후( terraform output / tailscale status )
# AWS_BASTION_TS_IP 를 채우고 ENABLE_VXLAN=true 로 재실행한다.
ENABLE_VXLAN="false"
VNI=100                               # docker 수업 memo4 와 동일 VNI (양쪽 일치 필수)
...
#    repo 는 rhel/9 사용 — Tailscale 바이너리는 정적 링크라 Rocky8 에서도 동작(project1 검증).
...
#       그래도 VXLAN 은 정상 동작함 — VXLAN 은 "서브넷 경로"가 아니라
#       Bastion 의 노드 IP(100.x)로 직접 연결되며, 노드 간 100.x 메시는
#       accept-routes 와 무관하게 항상 살아있기 때문.
...
sudo tailscale up \
    ...
    --reset                        # ★수정② : --hostname=proj-mgmt → "$TS_HOSTNAME"
...
if [ -n "${TAILSCALE_API_KEY:-}" ] && [ -n "${TAILNET_NAME:-}" ]; then
    ...
    if [ -n "$DEV_ID" ]; then
        curl -s -u "${TAILSCALE_API_KEY}:" -X POST \
            ...
            -d '{"routes":["172.16.1.0/24"]}' >/dev/null \
            && echo "[OK] 172.16.1.0/24 라우트 자동 승인 완료" \
...
# 7. (Opt 2) Tailscale 인터페이스 위에 VXLAN 오버레이 구성
#    [핵심] dev 를 물리 NIC(ens160) 가 아니라 tailscale0 로 지정한다.
#           → VXLAN 캡슐 패킷이 Tailscale 암호화 터널을 타고 나간다(= 평문 노출 방지).
#    [대칭성] 양쪽 노드가 같은 VNI(100), 같은 dstport(4789) 여야 하고,
#             local/remote 만 서로 교차(내 TS IP ↔ 상대 TS IP)된다.
if [ "$ENABLE_VXLAN" = "true" ]; then
    if [ -z "$AWS_BASTION_TS_IP" ]; then
        echo "[Error] AWS_BASTION_TS_IP 가 비어있음. Bastion 의 Tailscale IP 를 채운 뒤 재실행하세요."
        echo "        (Bastion 에서: tailscale ip -4 / 또는 terraform output bastion_ts_ip)"
        exit 1
    fi
    TS_LOCAL=$(tailscale ip -4 | head -1)   # 내(이 머신) Tailscale IP = VXLAN local

    if ! ip link show vxlan0 &>/dev/null; then
        echo "[INFO] vxlan0 생성 (VNI=$VNI, over tailscale0)"
        # id      : VNI (양쪽 동일해야 함)
        # remote  : 상대 노드(Bastion)의 Tailscale IP
        # local   : 내 Tailscale IP
        # dev     : tailscale0 (← 암호화 터널을 운송 경로로 사용)
        # dstport : 4789 (VXLAN 표준 UDP 포트)
        sudo ip link add vxlan0 type vxlan id "$VNI" \
            remote "$AWS_BASTION_TS_IP" local "$TS_LOCAL" dev tailscale0 dstport 4789
        # 오버레이 IP 부여 (proj-mgmt = .1, Bastion = .2)
        sudo ip addr add "$OVERLAY_LOCAL_IP" dev vxlan0
        sudo ip link set vxlan0 up
        echo "[OK] vxlan0 up — overlay $OVERLAY_LOCAL_IP ↔ Bastion 10.10.10.2"
    else
        echo "[OK] vxlan0 이미 존재"
    fi

    echo ""
    echo "📌 AWS Bastion 측 대칭 구성(예시 — Ansible 로 자동화 예정):"
    echo "   sudo ip link add vxlan0 type vxlan id $VNI \\"
    echo "        remote $TS_LOCAL local <Bastion_TS_IP> dev tailscale0 dstport 4789"
    echo "   sudo ip addr add 10.10.10.2/24 dev vxlan0 && sudo ip link set vxlan0 up"
    echo ""
    echo "📌 연결 테스트: ping 10.10.10.2   (Bastion 오버레이 IP)"
else
    echo ""
    echo "[SKIP] VXLAN 미적용 (ENABLE_VXLAN=false)."
    echo "       Bastion 의 Tailscale IP 확보 후 스크립트 상단 ENABLE_VXLAN=true,"
    echo "       AWS_BASTION_TS_IP 채우고 재실행하면 오버레이가 구성됩니다."
fi
